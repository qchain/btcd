// Copyright (c) 2015-2016 The qchain developers
	...
	tests := []struct {
	...
	}{
		{
			...
			func() (*peer.Peer, *peer.Peer, error) {
				inConn, outConn := pipe(
					&conn{raddr: "10.0.0.1:8444"},
					&conn{raddr: "10.0.0.2:8444"},
				)
				inPeer := peer.NewInboundPeer(peerCfg)
				if err := inPeer.Connect(inConn); err != nil {
					return nil, nil, err
				}
				outPeer, err := peer.NewOutboundPeer(peerCfg, "10.0.0.2:8444")
				if err != nil {
					return nil, nil, err
				}
		...
		},
		{
			...
			func() (*peer.Peer, *peer.Peer, error) {
				inConn, outConn := pipe(
					&conn{raddr: "10.0.0.1:8444", proxy: true},
					&conn{raddr: "10.0.0.2:8444"},
				)
				inPeer := peer.NewInboundPeer(peerCfg)
				if err := inPeer.Connect(inConn); err != nil {
...
func TestPeerListeners(t *testing.T) {
	...
	outPeer, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:8444")
	if err != nil {
...
func TestOutboundPeer(t *testing.T) {
	...
	r, w := io.Pipe()
	c := &conn{raddr: "10.0.0.1:8444", Writer: w, Reader: r}

	p, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:8444")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
	...
	r1, w1 := io.Pipe()
	c1 := &conn{raddr: "10.0.0.1:8444", Writer: w1, Reader: r1}
	p1, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:8444")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
	...
	r2, w2 := io.Pipe()
	c2 := &conn{raddr: "10.0.0.1:8444", Writer: w2, Reader: r2}
	p2, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:8444")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
