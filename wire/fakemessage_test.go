...
// MsgDecode doesn't do anything.  It just satisfies the wire.Message
// interface.
func (msg *fakeMessage) MsgDecode(r io.Reader, pver uint32) error {
	return nil
}

// MsgEncode writes the payload field of the fake message or forces an error
// if the forceEncodeErr flag of the fake message is set.  It also satisfies the
// wire.Message interface.
func (msg *fakeMessage) MsgEncode(w io.Writer, pver uint32) error {
	if msg.forceEncodeErr {
		err := &wire.MessageError{
			Func:        "fakeMessage.MsgEncode",
