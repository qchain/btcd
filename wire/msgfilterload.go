...
func (msg *MsgFilterLoad) MsgDecode(r io.Reader, pver uint32) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterload message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterLoad.MsgDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgFilterLoad) MsgEncode(w io.Writer, pver uint32) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filterload message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterLoad.MsgEncode", str)
	}
