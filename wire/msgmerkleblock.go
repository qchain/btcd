// Copyright (c) 2014-2015 The qchain developers
...
func (msg *MsgMerkleBlock) MsgDecode(r io.Reader, pver uint32) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("merkleblock message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgMerkleBlock.MsgDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgMerkleBlock) MsgEncode(w io.Writer, pver uint32) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("merkleblock message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgMerkleBlock.MsgEncode", str)
	}
