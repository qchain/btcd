...
func (msg *MsgNotFound) MsgDecode(r io.Reader, pver uint32) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgNotFound.MsgDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgNotFound) MsgEncode(w io.Writer, pver uint32) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgNotFound.MsgEncode", str)
	}
