// Copyright (c) 2016 The qchain developers
...
func (msg *MsgSendHeaders) MsgDecode(r io.Reader, pver uint32) error {
	if pver < SendHeadersVersion {
		str := fmt.Sprintf("sendheaders message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgSendHeaders.MsgDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgSendHeaders) MsgEncode(w io.Writer, pver uint32) error {
	if pver < SendHeadersVersion {
		str := fmt.Sprintf("sendheaders message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgSendHeaders.MsgEncode", str)
	}
