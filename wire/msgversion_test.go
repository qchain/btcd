...
func TestVersion(t *testing.T) {
	...
	tcpAddrMe := &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 8444}
	me, err := wire.NewNetAddress(tcpAddrMe, wire.SFNodeNetwork)
	if err != nil {
		t.Errorf("NewNetAddress: %v", err)
	}
	tcpAddrYou := &net.TCPAddr{IP: net.ParseIP("192.168.0.1"), Port: 8444}
	...
	conn = &fakeConn{
		localAddr:  &net.UDPAddr{IP: net.ParseIP("127.0.0.1"), Port: 8444},
	...
	conn = &fakeConn{
		localAddr:  tcpAddrMe,
		remoteAddr: &net.UDPAddr{IP: net.ParseIP("192.168.0.1"), Port: 8444},
	}
	msg, err = wire.NewMsgVersionFromConn(conn, nonce, lastBlock)
	if err != wire.ErrInvalidNetAddr {
		t.Errorf("NewMsgVersionFromConn: expected error not received "+
...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.MsgDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.MsgDecode(fr, pver); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.MsgDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		if _, ok := err.(*wire.MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("MsgEncode #%d wrong error got: %v, "+
		...
		err = msg.MsgDecode(buf, test.pver)
		...
		if _, ok := err.(*wire.MessageError); !ok {
			if err != test.readErr {
				t.Errorf("MsgDecode #%d wrong error got: %v, "+
...
var baseVersionEncoded = []byte{
	...
	0x20, 0xfc, // Port 8444 in big-endian
